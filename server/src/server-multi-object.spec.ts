...
describe('0 - lifecycle of a Controller', () => {
    ...
    it(`0.1 - connect a Controller to a MobileObjectServer and check that 1 MobileObject has been created and that it is turned off
        ...
        const sockets = new Subject<ISocketObs>();
        ...
        .subscribe(
            message => mobileObjectId = message.message
        )
        ...
        setTimeout(() => {
            ...
            mobileObjectServer.getMobileObject(mobileObjectId).isTurnedOnObs.subscribe(
                isTurnedOn => {
                    if (isTurnedOn) {
    ...
    it(`0.3 - connect a Controller to a MobileObjectServer and then a Monitor connects
        ...
        controllerSocket.sendSubject.pipe(take(1)).subscribe(data => mobileObjectId = data.message);
        // emits to simulate that a BIND_CONTROLLER has been received over this socket
        controllerSocket.bindAsController();
        ...
        setTimeout(() => {
            const command: MobileObjectCommandMessage = {action: MobileObjectCommand.TURN_ON};
