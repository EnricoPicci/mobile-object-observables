...
describe('check if this is an HOT observable', () => {
    ...
    it(`5.1.1 - a client subscribes to a mobile ojiect, then unsubscribes after 1 sec and then subscribes again after 2 sec
        ...
        mobileObject.accelerateX(acceleration);
        mobileObject.accelerateY(acceleration);
        const subscription0 = mobileObject.dynamicsObsX.pipe(combineLatest(mobileObject.dynamicsObsY)).subscribe();
        const subscription1 = mobileObject.dynamicsObsX.pipe(combineLatest(mobileObject.dynamicsObsY))
        .subscribe(
            data => {
                speedX = data[0].vel;
                speedY = data[1].vel;
        ...
        let subscription2;
        setTimeout(() => {
            subscription2 = mobileObject.dynamicsObsX.pipe(combineLatest(mobileObject.dynamicsObsY)).pipe(
                take(1),
            )
            .subscribe(
                data => {
