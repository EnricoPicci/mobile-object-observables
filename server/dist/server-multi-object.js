...
class MobileObjectServer {
    ...
    start(ISocketObservable) {
        ISocketObservable.pipe(operators_1.tap(() => console.log('Connected client.')), operators_4.mergeMap(socket => 
        ...
        operators_2.map(() => (socket) => this.handleMonitorObs(socket)), operators_3.finalize(() => console.log('BIND_MONITOR subscription completed'))), socket.onMessageType(MessageType.BIND_CONTROLLER)
        ...
        operators_2.map(() => (socket) => this.handleControllerObs(socket)), operators_3.finalize(() => console.log('BIND_CONTROLLER subscription completed'))))
