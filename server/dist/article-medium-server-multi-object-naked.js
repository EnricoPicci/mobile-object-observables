...
class MobileObjectServer {
    ...
    handleMonitorObs(socket) {
        ...
        const mobObjAdded = rxjs_1.merge(rxjs_1.from(this.mobileObjects).pipe(operators_1.map(([mobObjId, mobObj]) => ({ mobObj, mobObjId }))), this.mobileObjectAdded)
            .pipe(operators_1.tap(mobObjInfo => socket.send(MessageType.MOBILE_OBJECT, mobObjInfo.mobObjId)), operators_1.mergeMap(mobObjInfo => this.handleDynamicsObs(socket, mobObjInfo.mobObj, mobObjInfo.mobObjId)));
    ...
    handleDynamicsObs(socket, mobObj, mobObjId) {
        return mobObj.dynamicsObs
            .pipe(operators_1.tap(data => socket.send(MessageType.DYNAMICS_INFO + mobObjId, JSON.stringify(data))), operators_1.takeUntil(this.stopSendDynamicsInfo(socket, mobObjId)));
    }
    handleControllerObs(socket) {
        ...
        return rxjs_1.merge(commands, turnOn).pipe(operators_1.takeUntil(disconnect));
    }
    ...
    sendTurnedOnInfoObs(socket, mobObj, mobObjId) {
        return mobObj.isTurnedOnObs.pipe(operators_1.tap(isOn => socket.send(MessageType.TURNED_ON + mobObjId, JSON.stringify(isOn))));
    }
...
//# sourceMappingURL=article-medium-server-multi-object-naked.js.map